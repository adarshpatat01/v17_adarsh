...
patch(PosDB.prototype, {
    ...
    add_products(products) {
        ...
        for (var i = 0, len = products.length; i < len; i++) {
            ...
            var id=product.tag_ids;
            for (const tgs of product.tag_ids) {
                if(self.product_by_tag_id){
                    if(self.product_by_tag_id[tgs]){
                        self.product_by_tag_id[tgs].push(product);
                    }else{
                        self.product_by_tag_id[tgs] = [product];
                    }
                }
            }
